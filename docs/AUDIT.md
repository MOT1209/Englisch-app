...
| المرحلة | العنوان | يعالج | الحالة |
|---------|---------|-------|--------|
| **0** | تثبيت البناء: Gradle Wrapper، حذف اختبارات السقالة الميتة، إصلاح توقيع debug، تثبيت لغة الـJVM | C1–C4, C12, C13 | ✅ `209b57c8` |
| **1** | إيقاف فقدان البيانات: بذور لمرة واحدة، هجرة Room + `exportSchema`، ثمانية فهارس | C9, C10, H9 | ✅ `a776f6d0` |
| **2** | تدفّق الدرس: إصلاح الدرس غير القابل للإنهاء والخروج منه، منطق تصحيح مُختبَر | C8, C14, H4, M1 | ✅ `9432b599` |
| **3** | الصدق: تصحيح اسم النموذج، إظهار الأخطاء بدل تزييف النجاح، حذف درجة النطق المزيّفة وإذن الميكروفون | C6, C7, H6 | ✅ `478025ea` |
| **4** | التنقّل: `navigation-compose` بمكدّس رجوع حقيقي | H1 | ✅ `3b11a27f` |
| **5** | الاستمرارية: DataStore للثيم وسرعة الصوت | H5 | ✅ `b8765f62` |
| **6** | المعمارية: حَقن اعتماديات + واجهات للمستودع وخدمة الذكاء + ViewModels لكل شاشة | H2, H3 | ⬜ متبقٍّ |
| **7** | الثيم والتدويل: نقل الألوان المثبّتة، إصلاح الوضع الداكن، استخراج السلاسل، دعم RTL، حماية لوحة الإدارة | M2, H7, H8, H10 | ⬜ متبقٍّ |
| **8** | التنظيف: إزالة الاعتماديات غير المستخدمة، إصلاح Moshi + ProGuard، تفعيل التصغير، حذف الشيفرة الميتة، مفاتيح أجنبية | M4, M5, C11, H11, L1–L5 | ⬜ متبقٍّ |

**لا يُصلَح بالشيفرة وحدها:** C5 (مفتاح Gemini داخل الـAPK) يحتاج خادمًا وسيطًا أو Firebase AI Logic مع App Check و`google-services.json`.
